...
- **Image & text attachments** — attach an image or a text file with `@path` in your message (e.g. `@screenshot.png`, `@README.md`): images (PNG/JPEG/GIF/WebP, up to 20 MB, magic-byte validated) become multimodal `image_url` parts, text files are inlined as text. Only images and text files are supported as input. Paste an image from the clipboard to attach it too (macOS `pngpaste`, Wayland `wl-paste`, X11 `xclip`, Windows PowerShell — no extra dependencies). Path supports web-url for image as well.
