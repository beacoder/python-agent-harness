...
from tests.support import session_sandbox  # noqa: F401,E402  (side-effect: redirect SESSION_DIR)
...
class TestSubagentIsolation(unittest.TestCase):
    ...
    def test_plan_reminder_injected_once(self):
        ...
        from python_agent_harness.core.models import AgentMode

        client = RecClient(["sub done"])
        s = make_session(client)
        s.plan_mode.set_mode(
            AgentMode.PLAN,
            {
                "plan": "P",
                "plan-mode": "PM ${planInfo}",
                "build-switch": "B",
            },
        )
        ...
        count = sum(m.get("content") == reminder for payload in payloads for m in payload)
