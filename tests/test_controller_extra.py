...
from python_agent_harness.controller import Controller
from python_agent_harness.models import Message, TextPart
...
class TestSubmitWithoutContent(unittest.TestCase):
    def test_only_failed_references_returns_none(self):
        ...
        from python_agent_harness.attachments import AttachmentError

        ctrl = Controller(FakeSession())
        err = AttachmentError("missing.txt", "file not found: missing.txt")
        with mock.patch(
            "python_agent_harness.controller.parse_at_references",
            return_value=("", [], [err]),
        ):
...
class TestRunWorkerGuards(unittest.TestCase):
    ...
    def test_skips_history_adoption_when_no_messages(self):
        ...
        with mock.patch("python_agent_harness.controller.run_agent_loop") as loop:
