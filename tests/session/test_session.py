...
from python_agent_harness.core.models import Message, Usage
from python_agent_harness.session import config
from python_agent_harness.session.session import (
    Session,
    find_context_dir,
    find_skill_dir,
)
...
from tests.support.agent_test_utils import FakeClient, RecordingSession
...
class TestSubagentDedicatedClient(unittest.TestCase):
    ...
    def _run_concurrently(self, session, n=2):
        ...
        with mock.patch(
            "python_agent_harness.session.session.run_subagent", side_effect=fake_run_subagent
        ):
            threads = [
