...
class LSPClient:
    ...
    def start(self) -> None:
        ...
        self._reader = threading.Thread(target=self._read_loop, daemon=True, name=f"lsp-reader:{self.command[0]}")
        self._reader.start()
        try:
            result = self.request(
                "initialize",
    ...
    def request(self, method: str, params: Any = None) -> Any:
        ...
        if kind == "error":
            ...
            msg = value.get("message", "unknown LSP error") if isinstance(value, dict) else str(value)
            ...
            while not self._closed:
                ...
                while True:
                    ...
                    if b":" in line:
                        ...
                        headers[key.decode("ascii", "replace").lower()] = value.strip().decode("ascii", "replace")
                try:
        ...
        if "id" in message and ("result" in message or "error" in message):
            ...
            if isinstance(request_id, int):
                ...
                if waiter is not None:
                    waiter.put(("error", message["error"]) if "error" in message else ("result", message.get("result")))
            return
