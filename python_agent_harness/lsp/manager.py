...
def _find_root(path: str, project_dir: str) -> str:
    ...
    markers = {".git", "pyproject.toml", "package.json", "Cargo.toml", "go.mod", "compile_commands.json"}
    while True:
...
def get_client(path: str, project_dir: str) -> tuple[LSPClient, str]:
    ...
    if spec is None:
        raise LSPError(f"No LSP server configured or installed for {Path(path).suffix or 'this'} file type.")
