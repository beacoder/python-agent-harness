...
class LSP(Tool):
    ...
    def run(self, args: dict, ctx: ToolContext) -> str:
        ...
        path = os.path.realpath(os.path.abspath(os.path.join(ctx.cwd, raw_path))) if not os.path.isabs(raw_path) else os.path.realpath(raw_path)
        ...
        try:
            ...
            if operation == "goToDefinition":
                result = client.request("textDocument/definition", {"textDocument": {"uri": uri}, "position": position})
            elif operation == "findReferences":
                result = client.request("textDocument/references", {"textDocument": {"uri": uri}, "position": position, "context": {"includeDeclaration": True}})
                ...
                result = client.request("textDocument/implementation", {"textDocument": {"uri": uri}, "position": position})
            elif operation == "prepareCallHierarchy":
                result = client.request("textDocument/prepareCallHierarchy", {"textDocument": {"uri": uri}, "position": position})
            else:
                prepared = client.request("textDocument/prepareCallHierarchy", {"textDocument": {"uri": uri}, "position": position})
                if not prepared:
